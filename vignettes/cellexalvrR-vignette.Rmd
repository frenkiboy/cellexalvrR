...
if ( ! file.exists("log2data.RData") ) {
   ...
   diff.proj = cellexalObj@drc$graph1
   save( diff.proj, file="diff.proj.RData")
   rm( diff.proj )
   
   ddr.proj = cellexalObj@drc$graph2
...
proj.list <- list(diffusion=diff.proj,DDRtree=ddr.proj)
```
...
```{r, fig.show='hold',eval=T}
dim(log2data)
cellvr <- MakeCellexaVRObj( Matrix::Matrix(log2data, sparse=T), drc.list=proj.list, specie="mouse", cell.meta=cell.ids, facs.data=NULL)
```
...
```{r,fig.show='hold',eval=T}
cell.vr.scr <- new("cellexalvrR",data=Matrix::Matrix(log2data,sparse=T),drc=list(diff=diff.proj))
cell.vr.scr
```
...
```{r,fig.show='hold',eval=T}
# unfortunatelky broken at the moment
...
This functionallity is currently not implemented as the Seurat format has changed lately.
...
```{r,eval=F}
cellvrObj <- seurat2cellexalvr(SeuratObj)
```
